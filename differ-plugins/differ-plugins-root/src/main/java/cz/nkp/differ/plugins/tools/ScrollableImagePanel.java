...
public class ScrollableImagePanel extends CustomComponent{
	...
	public ScrollableImagePanel(Component scaledImage, final Component fullImage,int scale,DifferPluginInterface parent) throws ScrollableImagePanelException{
	...
	private static final Button getImageLargerButton(final Component fullImage) throws ScrollableImagePanelException{
		...
		fullSizeButton.addListener(new Button.ClickListener() {
		    public void buttonClick(ClickEvent event) {
		        parent.getApplication().getMainWindow().addWindow(new FullSizeImageWindow(fullImage));
		    }
		});
